...
@{
	CrmSchemaSettings = [PSCustomObject]@{
        Path = "$projectRoot\Data\Schema\LiquorTESTKristinDataSchema.xml"
        Destination = "$projectRoot\Data\Schema\LiquorTESTKristinDataSchema2.xml"
	...
	ExtractData = [PSCustomObject]@{
        ZipFile = "$projectRoot\temp\export\LCLBPortal\LCLBPortalKristinData.zip"
        Folder = "$projectRoot\data\LCLBPortalKristinTestData"
    }
}