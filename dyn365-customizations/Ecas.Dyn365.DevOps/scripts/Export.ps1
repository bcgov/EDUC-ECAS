param (
    ...
    [Parameter(Mandatory)]
    [ValidateSet('OnlineDev-ECAS','OnlineTest-ECAS', 'OnpremiseDEV-ECAS','OnpremiseUAT-ECAS','OnpremisePROD-ECAS')] # update this list based on files in the CrmConnectionParameters folder
    [string]
    $CrmConnectionName,

    # The settings for the actions performed during the export
    [Parameter(Mandatory)]
    [ValidateSet("All", "ECASCustomizations", "ECASSecurityRolesOnly", "ECASReportsOnly")] # update this list based on files in the ExportSettings folder
    [string]
    $ExportSettings,
